...
👩‍💻 My research follows two lines, <strong>multimodal generative models</strong> and <strong>multimodal contrastive learning</strong>. Both are ultimately driven by the same goal: learning informative representations that capture the semantics shared across modalities while preserving what is specific to each. I study both from a probabilistic and optimization perspective, focusing on the inference and optimization challenges that arise when <strong>scaling the number of modalities</strong>. Previously, I worked on implicit models such as neural ODEs.
</div>
